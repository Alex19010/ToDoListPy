...
Этот проект предназначен для тех, кто хочет создавать свои заметки в Telegram. В нем можно легко создавать записи и планы на день.
...
После установки библиотек нужно создать бота в Telegram. Для этого выполните следующие шаги:
...
В проекте также используется база данных `MySQL`. Вам необходимо будет создать базу данных и `user` для управления базой данных. В файле `connector.py` есть подключение к вашей базе данных. Вам нужно будет лишь создать файл `.env` в основной папке и записать туда данные. Все таблички в базе данных будут создаваться автоматическию

Инструкцию по созданию пользователя для MySQL можно найти на этом сайте:
